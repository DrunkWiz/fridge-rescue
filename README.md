...
| **Seeds + shop** 🌱 | In-game currency spent in the Shop tab: 48 items in seven groups: **full outfits** (T-rex, frog, penguin, strawberry, avocado, astronaut) that recolour Sprout's whole body and come with a matching backdrop (a volcano for the T-rex, a pond for the frog…), hats, face, neck, floating, pals that sit beside Sprout, and backdrops drawn behind it. One item per group at a time | +2 per item rescued, +5 per item donated, +1 per daily fridge check, +10 per badge, +20 per growth stage, +3 per meal-diary photo, +3 for skipping a double-buy on the shopping list, +5 for sharing your impact (weekly), weekly challenge rewards, and an optional rewarded ad (below). **Seeds can't be bought with money** |
